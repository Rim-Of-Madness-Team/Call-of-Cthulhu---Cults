// RimWorld.Planet.TransportPodsArrivalAction_GiveGift
...
public class PawnFlyerArrivalAction_GiveGift : PawnFlyerArrivalAction
{
    ...
    public PawnFlyerArrivalAction_GiveGift()
    {
    }

    public PawnFlyerArrivalAction_GiveGift(Settlement settlement)
    {
            ...
            for (int j = 0; j < pods[i].innerContainer.Count; j++)
            {
                if (pods[i].innerContainer[j] is Pawn pawn)
                {
                    if (pawn.RaceProps.Humanlike)
                    {
                        ...
                        else if (PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_FreeColonists
                                 .TryRandomElement(out result))
                        {
                            Find.HistoryEventsManager.RecordEvent(new HistoryEvent(HistoryEventDefOf.SoldSlave,
                                result.Named(HistoryEventArgsNames.Doer)));
                        }
                    }
                    else if (pawn.RaceProps.Animal && pawn.relations != null)
                    {
                        Pawn firstDirectRelationPawn =
                            pawn.relations.GetFirstDirectRelationPawn(PawnRelationDefOf.Bond);
    ...
    public static FloatMenuAcceptanceReport CanGiveGiftTo(IEnumerable<IThingHolder> pods, Settlement settlement)
    {
        ...
        return settlement != null && settlement.Spawned && settlement.Faction != null &&
               settlement.Faction != Faction.OfPlayer && !settlement.Faction.def.permanentEnemy && !settlement.HasMap;
    }
        ...
        IEnumerable<IThingHolder> pods, Settlement settlement)
    {
        ...
        return PawnFlyerArrivalActionUtility.GetFloatMenuOptions(() => CanGiveGiftTo(pods, settlement),
            () => new PawnFlyerArrivalAction_GiveGift(settlement),
            ...
            settlement.Tile, delegate(Action action)
            {
