...
namespace CultOfCthulhu
{
    public class SpellWorker_SleeperOfNKai : SpellWorker
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            Utility.SpawnThingDefOfCountAt(of: CultsDefOf.Cults_SleepTotem, count: 1,
                target: new TargetInfo(cell: altar(map: map).RandomAdjacentCell8Way(), map: map));

            //Spawn a 
            Messages.Message(text: "Cults_SleepTotem_Spawns".Translate(), def: MessageTypeDefOf.PositiveEvent);

            Utility.ApplyTaleDef(defName: "Cults_SpellSleeperOfNKai", map: map);
            return true;
        }
    }
}