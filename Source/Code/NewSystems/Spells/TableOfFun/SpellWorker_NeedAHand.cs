...
namespace CultOfCthulhu
{
    public class SpellWorker_NeedAHand : SpellWorker
    {
        ...
        {
            var list = PawnsToTransmogrify(map: map).InRandomOrder();
            return !list.TryRandomElement(result: out var pawn) ? null : pawn;
        }
        ...
        public IEnumerable<Pawn> PawnsToTransmogrify(Map map)
        {
            ...
            var one = from Pawn peeps in map.mapPawns.FreeColonistsSpawned
                where peeps.RaceProps.Humanlike && peeps.Faction == Faction.OfPlayer && !peeps.Dead &&
                      (peeps.Downed || peeps.InBed()) && !peeps.health.capacities.CapableOf(capacity: PawnCapacityDefOf.Moving)
                select peeps;
            if (one.Any())
            {
                return one;
        ...
        {
            var pawn = TestPawn(map: (Map) parms.target);
            ...
            pawn.health.AddHediff(def: CultsDefOf.Cults_TentacleArm, part: tempRecord);
            Messages.Message(
                text: pawn.LabelShort + "'s " + tempRecord.def.label +
                      " has been replaced with an otherworldly tentacle appendage.", def: MessageTypeDefOf.PositiveEvent);
